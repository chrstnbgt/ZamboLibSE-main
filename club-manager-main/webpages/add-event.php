...
if (isset($_POST['save'])) {
    ...
    if (hasConflictingEvent($event->eventStartDate . ' ' . $event->eventStartTime, $event->eventEndDate . ' ' . $event->eventEndTime, $event->eventStreetName, $event->eventBuildingName, $existingEvents)) {
...
<body>
    ...
    <div class="input-group flex-column mb-3">
        ...
        <br>
        <?php
        // Retrieve session librarian ID
        $sessionLibrarianID = $_SESSION['librarianID'];

        foreach ($librarians as $librarian) {
            // Check if the current librarian ID matches the session librarian ID
            $isChecked = $librarian['librarianID'] == $sessionLibrarianID ? 'checked' : '';
...
<!-- 
                        ...
                        <div class="modal-action-btn d-flex justify-content-end"style="margin-bottom:100px;">
